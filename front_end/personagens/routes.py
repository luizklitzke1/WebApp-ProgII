...
def procurar_personagem():
    ...
    #Se nenhum filtro for informado:
     ...
     #Request do jsond do back-end
    pers_request = requests.get('http://localhost:5000/procurar_personagem/')
    
    #Converter json para dicionario Py
    pers_json = pers_request.json()
    
    #Converter em uma lista em Py
    pers_py = []
    for pers in pers_json:
        #Nova pessoa como a classe
        per = Personagem(pers)
        pers_py.append(per)                  
